...
We calculate $\beta_i$ and $\alpha_i$ from regression, and we assume the error $\epsilon$ is independent and normally distributed, i.e., $\text{Cov}(\epsilon_{i}, \epsilon_{j}) = 0$, $\text{Cov}(\hat{r}_m, \epsilon_{i}) = 0$, and $\mathbb{E}[\epsilon] = 0$, $\text{Var}(\epsilon) = \sigma_{i}^2$. Given this, we let the shrinkage matrix be:

$$
F = \beta \beta^{T} \hat{\sigma}_{m}^2 + \Sigma_{\epsilon}
$$
