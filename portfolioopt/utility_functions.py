"""
The ``utility_functions`` module contains some usfeul utility functions.
Due to the scipy.optimize standards, the custom utility function must accept
``weights`` as an argument, and must also have at least one of ``mean`` or ``cov``.
Because scipy.optimize only minimises, any utility that we want to maximise must be
made negative.
